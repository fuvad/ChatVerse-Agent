...
const Home = () => {
  ...
  // Explicitly typed as string | null — without this, useState(null)
  // infers the type as `null` only, so the later
  // setCheckpointId(data.checkpoint_id) call (a string) would error.
  const [checkpointId, setCheckpointId] = useState<string | null>(null);

  // Typed to match the onSubmit prop InputBar expects
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (currentMessage.trim()) {
      ...
      try {
        ...
        let url = `https://chatverse-latest.onrender.com/chat_stream/${encodeURIComponent(userInput)}`;
        if (checkpointId) {
        ...
        let streamedContent = "";
        // Explicitly typed as SearchInfo | null — without this, TypeScript
        // infers `null` as the only possible type from the initial value,
        // so every later `searchData = newSearchInfo;` assignment errors
        // because it thinks searchData can never hold an object.
